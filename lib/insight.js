 ...
 * @constructor
 */
function Insight(url, network, prefix = '/api') {
  if (!url && !network) {
  ...
  endpointPrefix = prefix.slice(0,1) == '/' ? prefix : '/' + prefix;
  return this;
}
...
Insight.prototype.getTransaction = function(txid, callback) {
  ...
  this.requestGet(endpointPrefix + '/tx/' + txid, function(err, res, body) {
...
Insight.prototype.getUtxos = function(addresses, callback) {
  ...
  this.requestPost(endpointPrefix + '/addrs/utxo', {
...
Insight.prototype.broadcast = function(transaction, callback) {
  ...
  this.requestPost(endpointPrefix + '/tx/send', {
    rawtx: transaction
...
Insight.prototype.address = function(address, callback) {
  ...
  this.requestGet(endpointPrefix + '/addr/' + address.toString(), function(err, res, body) {
    if (err || res.statusCode !== 200) {
      return callback(err || body);
    }
    var info;
